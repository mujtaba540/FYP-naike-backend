...
exports.create = async (Data,username) => {
    try {
        ...
        await models.appointment.create(obj)
        return {
            response:true
        }
    } catch (error) {
 ...
 exports.update=async(Data,username)=>{
    try{
        await db.authenticate();
        var result=await models.appointment.findOne({where:{AppointmentID:Data.AppointmentID}})
        if(result==null){
            ...
            await models.appointment.update(obj,{where :{AppointmentID:Data.AppointmentID}})
 ...
 exports.delete=async(id,username)=>{
    try{
        await db.authenticate()
            var check=await models.appointment.findOne({where:{AppointmentID:id}})
            if(check==null){
                return {resposne:false,error:new APIError({
                    message:"NOT FOUND",
                    status:httpStatus.NOT_FOUND
               ...
               await models.appointment.update({DeletedBy:username,DeletedOn:new Date(),Active:false},
               {where :{
               appointmentID:id,
               Active:true}
           })
...
exports.id=async(id)=>{
    try{
        await db.authenticate();
        var result=await models.appointment.findOne({
